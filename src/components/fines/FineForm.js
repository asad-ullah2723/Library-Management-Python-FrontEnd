...
import { DialogTitle, DialogContent, DialogActions, TextField, Button, Grid, Alert, MenuItem } from '@mui/material';
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, { setSubmitting, setErrors }) => {
        try {
          if (fine && fine.id) {
            await fineService.updateFine(fine.id, values);
          } else {
            await fineService.createFine(values);
          }
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              <Grid item xs={12} sm={6}>
                <TextField
                  label="Fine ID"
                  name="fine_id"
                  value={values.fine_id}
                  onChange={handleChange}
                  onBlur={handleBlur}
                  fullWidth
                  error={touched.fine_id && !!errors.fine_id}
                  helperText={touched.fine_id && errors.fine_id}
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField
